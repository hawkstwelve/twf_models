...
class HRRRCacheConfig:
    base_dir: Path = field(default_factory=default_hrrr_cache_dir)
    keep_runs: int = 1
...
def list_day_dirs(cfg: HRRRCacheConfig) -> list[Path]:
...
def list_cycle_dirs(cfg: HRRRCacheConfig) -> list[Path]:
...
def get_latest_cycle_dir(cfg: HRRRCacheConfig) -> Path:
    cycles = list_cycle_dirs(cfg)
    if not cycles:
        raise FileNotFoundError(f"No HRRR cycle directories found in {cfg.base_dir}")
    return cycles[0]
...
def resolve_hrrr_grib_path(cfg: HRRRCacheConfig, run: str, fh: int) -> Path:
    ...
    if not matches:
        ...
        raise FileNotFoundError(
            ...
            f"Searched for tokens: {wrf_token}, {fh_token}\n"
...
def enforce_cycle_retention(cfg: HRRRCacheConfig) -> dict[str, int]:
    ...
    for cycle_dir in delete_dirs:
        ...
        logger.info("Deleting HRRR cycle directory: %s", cycle_dir)

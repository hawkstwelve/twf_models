...
class Settings(BaseSettings):
    ...
    forecast_hours: str = "0,3,6,9,12,15,18,21,24,27,30,33,36,39,42,45,48,51,54,57,60,63,66,69,72,75,78,81,84,87,90,93,96,99,102,105,108,111,114,117,120,126,132,138,144,150,156,162,168,174,180,186,192,198,204,210,216,222,228,234,240,246,252,258,264,270,276,282,288,294,300,306,312,318,324,330,336,342,348,354,360,366,372,378,384"  # 3h to 120h, then 6h to 384h (GFS)
    # AIGFS-specific: 6-hourly forecasts (AI model produces data every 6 hours)
    aigfs_forecast_hours: str = "0,6,12,18,24,30,36,42,48,54,60,66,72,78,84,90,96,102,108,114,120,126,132,138,144,150,156,162,168,174,180,186,192,198,204,210,216,222,228,234,240,246,252,258,264,270,276,282,288,294,300,306,312,318,324,330,336,342,348,354,360,366,372,378,384"  # 6h increments to f384
    ...
    @property
    def aigfs_forecast_hours_list(self) -> List[int]:
        """Parse AIGFS-specific forecast hours string into list"""
        return [int(h.strip()) for h in self.aigfs_forecast_hours.split(",")]

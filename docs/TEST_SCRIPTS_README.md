...
This directory contains test scripts for manual testing and development.
...
```bash
# From /Users/brianaustin/twf_models/
python scripts/tests/test_temp_map.py
```
...
- `run_latest_gfs_now.py` - Generate all maps for latest GFS run
- `run_latest_aigfs_now.py` - Generate all maps for latest AIGFS run  
- `run_latest_now.py` - Generate maps for all enabled models

### Individual Map Type Tests

- `test_temp_map.py` - Temperature map
- `test_precip_map.py` - Total precipitation
- `test_snowfall_map.py` - Total snowfall (10:1 ratio)
- `test_850mb_map.py` - 850mb temperature, wind, MSLP
- `test_mslp_precip_map.py` - MSLP with 6-hour precipitation
- `test_wind_speed_map.py` - 10m wind speed
- `test_radar_map.py` - Simulated radar reflectivity

### Herbie Integration Tests

**NEW:** Tests for evaluating Herbie package integration

- `test_herbie_comparison.py` - Compare NOMADS vs Herbie fetcher
  - Measures download speed
  - Validates data quality
  - Tests cache integration
  - Evaluates HerbieWait for progressive monitoring

**Usage:**
```bash
# Install Herbie first (optional, for testing only)
...
See `docs/HERBIE_INTEGRATION.md` for detailed analysis and recommendations.

## Notes

- Tests use the latest available model run
- Images are saved to `backend/images/` directory
- Tests require backend dependencies: `pip install -r backend/requirements.txt`
- Some tests may take 10-30 minutes depending on data availability

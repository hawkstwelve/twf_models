...
function setSelectOptions(select, items, { includeLatest = false } = {}) {
  ...
  if (includeLatest && items.length > 0) {
    ...
    if (latestItem) {
      ...
      rawValues = [{ id: "latest", label: latestLabel }, ...filtered.slice(1)];
    } else {
      rawValues = items;
    }
  }
...
async function fetchFrames({ model, region, run, varKey }) {
  ...
  try {
    ...
    const frameRows = Array.isArray(response)
      ? response
          .filter((row) => row && row.has_cog && Number.isFinite(Number(row.fh)))
          .sort((a, b) => Number(a.fh) - Number(b.fh))
      : [];
    const frames = frameRows.map((row) => Number(row.fh));
    const first = frameRows[0] ?? null;
    return {
      frames,
      frameRows,
    ...
    return { frames: [], frameRows: [], legendMeta: null };
  }
}
...
export async function initControls({
  ...
  async function updateFrames({ preserveFh, userInitiated }) {
    ({ frames, frameRows, legendMeta } = await fetchFrames({
      model: selectedModel,
      region: selectedRegion,
      run: selectedRun,
      varKey: selectedVar,
  ...
  if (fhSlider && fhDisplay) {
    fhSlider.addEventListener("input", (event) => {
      ...
      if (fhSlider.dataset.mode === "index") {
        try {
          const sliderFrames = JSON.parse(fhSlider.dataset.frames || "[]");
          const index = Number(event.target.value);
          if (Number.isInteger(index) && index >= 0 && index < sliderFrames.length) {
            nextFh = Number(sliderFrames[index]);
          }
        } catch (_) {
